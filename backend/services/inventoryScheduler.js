...
const runDailyInventoryUpdate = async () => {
  logger.info("⏱️ Starting Daily Inventory Update...");
  const now = new Date();
  ...
  try {
    ...
    if (expiredItems.length > 0) {
      ...
      sendAlert(
        `🚨 Daily Cleanup: ${expiredItems.length} items expired and were moved to waste (${totalExpiredWeight.toFixed(1)}kg total).`,
        "admin"
      );
      logger.info(`✅ Formatted ${expiredItems.length} expired items into waste logs.`);
    }
    ...
    logger.info(
      `✅ Successfully simulated daily consumption for ${depletedCount} inventory items.`
    );
    logger.info("✅ Daily Inventory Update Complete.");
  } catch (error) {
    logger.error("❌ Error during daily inventory update: %o", error);
  }
};
