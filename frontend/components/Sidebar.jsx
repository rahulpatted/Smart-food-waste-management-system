...
export default function Sidebar() {
  ...
  const navLinks = user?.role && roleLinks[user.role] ? roleLinks[user.role] : [
    ...
    <div className="hidden md:flex w-64 sticky top-16 h-[calc(100vh-64px)] bg-white/80 dark:bg-slate-900/60 backdrop-blur-3xl border-r border-slate-200 dark:border-white/5 text-slate-600 dark:text-slate-300 p-6 flex-col shadow-xl dark:shadow-[20px_0_50px_rgba(0,0,0,0.3)] shrink-0 z-50 overflow-hidden print:hidden">
      <div className="flex items-center gap-3 mb-10 px-2 group">
        ...
        <div className="flex flex-col">
          <h2 className="text-xs font-black tracking-[0.2em] text-slate-400 dark:text-white uppercase leading-none">Navigation</h2>
        ...
        {navLinks.map((link, idx) => (
          link.label ? (
            <li key={`lbl-${idx}`} className="pt-4 pb-2 px-5 first:pt-0">
              <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">{link.label}</span>
            </li>
          ) : (
            <SidebarLink 
              key={link.href + idx} 
          ...
          <>
            ...
            >
              ...
              <div className="overflow-hidden">
                <p className="text-sm font-bold text-slate-900 dark:text-white leading-tight capitalize truncate">{user.name || user.role}</p>
                <div className="flex items-center gap-1.5 mt-0.5">
                   <div className={`w-1.5 h-1.5 rounded-full ${pathname === "/profile" ? "bg-indigo-500" : "bg-emerald-500 animate-pulse"}`}></div>
            ...
            >
              <LogOut size={18} className={`${isLoggingOut ? 'animate-pulse' : 'group-hover:-translate-x-1'} transition-transform`} />
              <span className="text-xs">{isLoggingOut ? 'Signing Out...' : 'Sign Out'}</span>
              {!isLoggingOut && <ChevronRight size={14} className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity" />}
            </button>
          </>
        ) : (
          <div className="flex flex-col gap-3">
             <Link href="/login" className="w-full py-4 bg-emerald-600 text-white rounded-2xl text-xs font-black uppercase text-center tracking-widest hover:bg-emerald-700 shadow-xl shadow-emerald-600/20 transition-all">Log In</Link>
             <Link href="/register" className="w-full py-4 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 text-slate-600 dark:text-white rounded-2xl text-xs font-black uppercase text-center tracking-widest hover:bg-slate-100 dark:hover:bg-white/10 transition-all">Sign Up</Link>
          </div>
        )}
      </div>
    </div>
    ...
    <li>
      <Link href={href}>
        ...
        >
          ...
          <div className={`relative z-10 shrink-0 ${active ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400 dark:text-slate-600 group-hover:text-emerald-500"} transition-colors`}>
            <Icon size={20} strokeWidth={active ? 2.5 : 2} className={active ? "drop-shadow-[0_0_5px_rgba(16,185,129,0.5)]" : ""} />
          </div>

          <div className="relative z-10 overflow-hidden">
            <span className={`text-[11px] font-black uppercase tracking-[0.15em] italic whitespace-nowrap transition-all ${active ? "text-emerald-600 dark:text-emerald-400" : ""}`}>
              {text}
            </span>
          </div>
          
          {!active && (
            <ChevronRight size={14} className="ml-auto text-slate-300 dark:text-slate-600 opacity-0 group-hover:opacity-100 transition-all -translate-x-2 group-hover:translate-x-0" />
          )}
        </motion.div>
      </Link>
    </li>
