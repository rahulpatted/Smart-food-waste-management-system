    ...
    <div className="bg-white/90 dark:bg-slate-900/40 backdrop-blur-3xl rounded-[2.5rem] border border-white dark:border-white/5 p-8 shadow-2xl h-full relative overflow-hidden group">
      ...
      <div className="space-y-4 relative z-10">
        {leaders.map((leader, i) => (
          ...
          >
            <div
              className={`w-9 h-9 rounded-xl flex items-center justify-center font-black text-xs shadow-inner transition-transform group-hover/row:scale-105 ${i === 0 ? "bg-white text-indigo-600" : "bg-slate-100 dark:bg-slate-800/80 text-slate-500"}`}
            >
              {leader.rank}
            </div>
            <div className="flex-1 min-w-0">
              <p
                className={`text-xs font-black uppercase tracking-[0.1em] italic truncate ${i === 0 ? "text-white" : "text-slate-900 dark:text-white"}`}
              >
                {leader.name}
              </p>
              <div className="flex items-center gap-2 mt-1 opacity-80">
                ...
                <span className="text-[9px] font-black uppercase tracking-[0.1em]">
                  {leader.streak} Day Heat
                </span>
              </div>
            </div>
            <div className="text-right">
              <p
                className={`text-sm font-black tracking-tighter ${i === 0 ? "text-white" : "text-indigo-600 dark:text-indigo-400"}`}
              >
                {leader.pts.toLocaleString()}
              </p>
