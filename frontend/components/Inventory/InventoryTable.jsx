...
export default function InventoryTable(props) {
  ...
  const generateProcurement = () => {
    ...
    a.href = url;
    a.download = `procurement_order_${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
  ...
  const generateRecipes = () => {
    ...
    setTimeout(() => {
      setRecipes([
        {
          icon: "🥘",
          title: "Zero-Waste Veggie Hash",
          time: "15 mins",
          description:
            "A quick, high-nutrient breakfast hash using leftover bread and available vegetables.",
        },
        {
          icon: "🥪",
          title: "Savoury Bread Pudding",
          time: "30 mins",
          description: "Bake near-expiry bread with herbs, stock, and remaining vegetable scraps.",
        },
        {
          icon: "🍲",
          title: "Canteen Leftover Stew",
          time: "45 mins",
          description:
            "A slow-cooked stew that maximizes flavor from wilting greens and expiring items.",
        },
      ]);
  ...
  const getStatus = (item) => {
    ...
    if (diffHours > 0 && diffHours <= 24) {
      return {
        ...
        cls: "bg-rose-500 text-white animate-pulse shadow-[0_0_20px_rgba(244,63,94,0.5)] border-rose-400",
        critical: true,
      };
    }
  ...
  const filtered = (
    filter.toLowerCase() === "all"
      ? data
      : filter.toLowerCase() === "low"
        ? data.filter((i) => i.quantity <= LOW_STOCK_THRESHOLD)
        : filter.toLowerCase() === "expiring"
          ? data.filter((i) => isExpiringSoon(i.expiryDate) || isExpired(i.expiryDate))
          : data.filter((i) => i.category?.toLowerCase() === filter.toLowerCase())
  ).sort((a, b) => {
      ...
      {expiringItems.length > 0 && (
        <div className="bg-gradient-to-br from-indigo-900 via-indigo-800 to-purple-900 rounded-[2rem] p-1 text-white shadow-xl relative overflow-hidden animate-in fade-in slide-in-from-bottom-4 group">
          <div className="absolute top-0 right-0 p-8 opacity-10">
            <ChefHat
              size={120}
              className="transform rotate-12 group-hover:scale-110 transition-transform"
            />
          </div>
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  <h3 className="font-bold text-xl text-white flex items-center gap-2">
                    AI Chef Assistant
                    <span className="bg-indigo-500 text-white text-[10px] px-2 py-0.5 rounded-full font-black uppercase tracking-widest animate-pulse border border-indigo-400">
                      Beta
                    </span>
                  </h3>
                </div>
                <p className="text-indigo-200 text-sm mb-4 font-medium">
                  You have {expiringItems.length} items expiring very soon:{" "}
                ...
                {!recipes && !isGeneratingRecipes && (
                  <button
                    ...
                    className="flex items-center gap-2 bg-white text-indigo-900 px-5 py-2.5 rounded-xl text-sm font-black uppercase tracking-widest shadow-xl hover:scale-105 transition-all"
                  >
                  ...
                  <div className="mt-6 space-y-3">
                    {recipes.map((r, idx) => (
                      <motion.div
                        ...
                        key={r.title}
                        className="bg-black/30 border border-white/10 rounded-xl p-4 flex gap-4 items-start hover:bg-black/40 transition-colors cursor-pointer group"
                      >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-4">
        ...
        <div className="bg-white dark:bg-slate-800/60 backdrop-blur-sm p-8 rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-xl group hover:border-orange-500/30 transition-all hover:-translate-y-1">
          ...
          <p className="text-4xl font-black text-orange-500 tracking-tighter">
            {data.filter((i) => i.quantity <= LOW_STOCK_THRESHOLD).length}
          </p>
        ...
        <div className="bg-white dark:bg-slate-800/60 backdrop-blur-sm p-8 rounded-[2rem] border border-rose-500/20 shadow-xl group hover:border-rose-500/30 transition-all hover:-translate-y-1">
          <p className="text-[10px] font-black text-rose-500 uppercase tracking-[0.3em] mb-1 text-rose-500/80">
            Critical Rescue
          </p>
              ...
              {paginated.length === 0 ? (
                ...
                paginated.map((item) => {
                  ...
                  return (
                    <tr
                      key={item._id}
                      className={`hover:bg-emerald-50/30 dark:hover:bg-emerald-500/5 transition-colors group ${status.critical ? "border-l-4 border-rose-500 bg-rose-500/5" : ""}`}
                    >
                      <td className="py-5 px-8">
                        <div
                          className="relative w-12 h-12 cursor-pointer overflow-hidden rounded-2xl border-2 border-slate-100 dark:border-slate-700 group-hover:border-emerald-500/50 transition-all shadow-sm"
                          onClick={() => setExpandedQR({ url: qrUrl, name: item.item })}
                        >
                          <img
                            src={qrUrl}
                            alt="QR"
                            className="w-full h-full grayscale group-hover:grayscale-0 transition-opacity opacity-80 group-hover:opacity-100"
                          />
                      ...
                      <td className="py-5 px-8">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] font-black text-emerald-500 dark:text-emerald-500 uppercase tracking-widest mt-0.5 px-2 py-0.5 bg-emerald-500/10 rounded-md border border-emerald-500/20">
                      ...
                      <td className="py-5 px-8 text-right md:text-left">
                        <span
                          className={`inline-flex items-center px-4 py-1.5 rounded-xl text-xs font-black tracking-tight ${item.quantity <= LOW_STOCK_THRESHOLD ? "bg-orange-50 dark:bg-orange-500/10 text-orange-600 dark:text-orange-400 border border-orange-100 dark:border-orange-500/20" : "bg-slate-50 dark:bg-slate-700/60 text-slate-600 dark:text-slate-300 border border-slate-100 dark:border-white/5"}`}
                        >
                          {item.quantity} units
                        </span>
                      </td>
                      ...
                      <td className="py-5 px-8 text-right">
                        <span
                          className={`inline-flex items-center px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest ${status.cls}`}
                        >
                          {status.label}
                        </span>
                      </td>
          ...
          <div className="flex items-center justify-between px-8 py-5 border-t border-slate-100 dark:border-slate-700/60 bg-slate-50/30 dark:bg-transparent">
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={page === 1}
                className="px-5 py-2 text-xs font-black uppercase tracking-widest rounded-xl bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-200 disabled:opacity-30 hover:bg-slate-50 dark:hover:bg-slate-600 transition-all shadow-sm"
              >
                Prev
              </button>
              <button
        ...
        {expandedQR && (
          ...
          >
            <motion.div
              ...
              className="bg-white dark:bg-slate-800 rounded-3xl p-10 max-w-sm w-full text-center relative border border-white/10 shadow-2xl"
            ...
            >
              ...
              <p className="text-xs text-slate-500 font-bold mb-8 uppercase tracking-widest leading-none">
                Official Inventory Label
              </p>
              ...
              <button
                ...
                className="w-full py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl font-black uppercase text-xs tracking-tighter hover:scale-105 transition-all shadow-xl"
              >
                Print Label 🖨️
              </button>
            </motion.div>
