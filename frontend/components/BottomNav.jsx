...
import { LayoutDashboard, Utensils, Package, Handshake, Map as MapIcon, Shield, Settings, User } from "lucide-react";
    ...
    <div className="md:hidden fixed bottom-0 left-0 w-full bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-t border-slate-200 dark:border-slate-800 z-50 pb-safe print:hidden shadow-[0_-10px_40px_rgba(0,0,0,0.1)] dark:shadow-[0_-10px_40px_rgba(0,0,0,0.4)]">
      <div className="flex justify-around items-center h-16">
        {navLinks.map((link) => {
          ...
          return (
            <Link key={link.href} href={link.href} className={`flex flex-col items-center justify-center w-full h-full space-y-1 relative transition-all ${isActive ? "text-emerald-500" : "text-slate-400 dark:text-slate-500 hover:text-slate-800 dark:hover:text-slate-200"}`}>
              {isActive && (
                <div className="absolute -top-[1px] w-8 h-[2px] bg-emerald-500 rounded-b-full shadow-[0_2px_10px_rgba(16,185,129,0.5)]"></div>
              )}
              <Icon size={20} strokeWidth={isActive ? 2.5 : 2} className={isActive ? "scale-110 transition-transform" : "scale-100 transition-transform"} />
