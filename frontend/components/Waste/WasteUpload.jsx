...
export default function WasteUpload() {
  ...
  const exportCSV = () => {
    ...
    const rows = logs.map(
      (w) => `${w.type},${w.weight},${new Date(w.createdAt || w.date).toLocaleDateString()}`
    );
        ...
        <div className="bg-rose-50 dark:bg-rose-900/20 border-b border-rose-100 dark:border-rose-900/40 p-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-rose-700 dark:text-rose-400 text-sm font-medium">
              Capture excess for metrics generation
            </p>
          </div>
        </div>
            ...
            <div className="flex items-center gap-4">
              ...
              {photoPreview && (
                <motion.div
                  ...
                  className="relative w-24 h-24 rounded-2xl overflow-hidden border-2 border-rose-500 shadow-lg"
                >
                  <img
                    src={photoPreview}
                    alt="Waste Preview"
                  ...
                  <button
                    ...
                    className="absolute top-1 right-1 bg-white/80 rounded-full p-1 text-rose-500 hover:bg-white transition-colors"
                  >
                    <X size={12} />
                  </button>
                </motion.div>
          ...
          {coordinates && (
            <div className="w-full mt-2 animate-in fade-in zoom-in-95 duration-500">
              <div className="flex items-center justify-between mb-2">
                <p className="text-[10px] font-black text-rose-600 uppercase tracking-widest px-2 py-0.5 bg-rose-50 rounded-full border border-rose-100">
                  GPS Attached 📍
                </p>
                <button
                  type="button"
                ...
                {paginated.length === 0 ? (
                  <tr>
                    <td colSpan="4" className="py-8 text-center text-slate-400">
                      No logs yet. Add your first waste entry above!
                    </td>
                  </tr>
                ) : (
                  paginated.map((log) => (
                    <React.Fragment key={log._id}>
                      <tr className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                        <td className="py-3.5 px-6 font-medium text-slate-800 dark:text-slate-200 capitalize">
                          {log.type}
                        </td>
                        ...
                        <td className="py-3.5 px-6 text-slate-500 dark:text-slate-400">
                          ...
                          {log.coordinates?.lat && (
                            <button
                              onClick={() =>
                                setExpandedMap(expandedMap === log._id ? null : log._id)
                              }
                              title="Toggle Interactive Map"
                              className="ml-2 inline-flex items-center justify-center bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 rounded p-1 transition-colors"
                            >
                              🗺️
                            </button>
                          )}
                        ...
                        <td className="py-3.5 px-6 text-right">
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-bold ${log.type === "edible" ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}
                          >
                          ...
                          <td colSpan="4" className="p-4">
                            <div className="w-full h-[300px] z-0 rounded-xl overflow-hidden shadow-inner">
                              <MapWrapper
                                ...
                                popupText={`${log.weight}kg of ${log.type}`}
                              />
                            </div>
                          </td>
                        </tr>
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 dark:border-slate-700">
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={page === 1}
                className="px-3 py-1.5 text-sm rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 disabled:opacity-40 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
              >
                ← Prev
              </button>
              <button
