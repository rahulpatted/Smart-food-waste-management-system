    ...
    >
      ...
      <div className="relative z-10 h-full flex flex-col">
        <div className="flex items-center justify-between mb-10">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-400">
                Live Pulse
              </h3>
            </div>
            <h2 className="text-3xl font-black tracking-tighter italic bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">
              Today's Menu
            </h2>
          </div>
        </div>
          ...
          {Object.entries(todayMenu).map(([type, food], i) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl shadow-xl border border-white/5 transition-all duration-500 ${type === "Breakfast" ? "bg-orange-500/20 group-hover/item:border-orange-500/50" : type === "Lunch" ? "bg-blue-500/20 group-hover/item:border-blue-500/50" : "bg-indigo-500/20 group-hover/item:border-indigo-500/50"}`}
                >
        ...
        <div className="mt-10 p-5 bg-emerald-500/5 rounded-3xl border border-emerald-500/10 flex items-center gap-4 relative overflow-hidden group/tip">
          ...
          <div className="shrinking-0 text-emerald-500 relative z-10">
            <Info size={24} />
          </div>
    ...
    <div className="bg-white/90 dark:bg-slate-900/40 backdrop-blur-3xl rounded-[2.5rem] p-10 shadow-2xl border border-white dark:border-white/5 relative group h-full flex flex-col">
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div>
          ...
          <p className="text-[9px] font-black text-emerald-600 dark:text-emerald-500 uppercase tracking-[0.4em] mt-1.5 opacity-80">
            Commit to Zero-Waste Service
          </p>
        </div>

        <p className="text-slate-500 dark:text-slate-400 text-sm font-bold mb-10 leading-relaxed max-w-sm italic">
          Commit to precision kitchen operations by securing your portion. Every reservation
          prevents surplus and saves resources.
        </p>
        ...
        <AnimatePresence mode="wait">
          {success && (
            ...
            >
              ...
              <div>
                ...
                <p className="text-[10px] opacity-80 mt-0.5 tracking-tight">
                  Your impact is being tracked live.
                </p>
              </div>
            </motion.div>
          )}
          {error && (
            ...
            >
              <div className="w-12 h-12 rounded-full bg-rose-500 text-white flex items-center justify-center shadow-lg shadow-rose-500/20 text-lg">
                ⚠️
              </div>
        ...
        <form onSubmit={bookMeal} className="space-y-8 flex-grow flex flex-col justify-end">
          <div className="space-y-3">
            <label className="block text-[9px] font-black text-emerald-600/60 dark:text-emerald-400/60 uppercase tracking-[0.3em] ml-1">
              Choose Service Engagement
            </label>
            <div className="relative group/select">
              ...
              >
                ...
                <div className="flex items-center gap-3 relative z-10">
                  ...
                  <span className="font-black text-slate-900 dark:text-white uppercase text-sm tracking-[0.2em] italic drop-shadow-sm">
                    {mealType} Service
                  </span>
                </div>
                <motion.div
                  animate={{ rotate: dropdownOpen ? -90 : 90 }}
                    ...
                    {["Breakfast", "Lunch", "Dinner"].map((opt) => (
                      ...
                      >
                        {mealType === opt && (
                          <motion.div
                            layoutId="activeMeal"
                            className="absolute left-0 top-1/4 bottom-1/4 w-1 bg-emerald-500 rounded-r-full shadow-[0_0_8px_rgba(16,185,129,0.8)]"
                          ></motion.div>
                        )}
                        <div
                          className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg shadow-inner transition-all duration-500 ${opt === "Breakfast" ? "bg-orange-500/10 text-orange-500" : opt === "Lunch" ? "bg-blue-500/10 text-blue-500" : "bg-indigo-500/10 text-indigo-500"}`}
                        >
                          {opt === "Breakfast" ? "🌅" : opt === "Lunch" ? "🍱" : "🌙"}
                        </div>
                        <span
                          className={`font-black uppercase text-xs tracking-[0.2em] italic ${mealType === opt ? "text-emerald-600 dark:text-emerald-400" : "text-slate-600 dark:text-slate-400"}`}
                        >
                          {opt} Service
                        </span>
                      </div>
