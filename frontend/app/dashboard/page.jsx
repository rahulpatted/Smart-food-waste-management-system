...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchStats = () => {
      ...
      Promise.all([
        ...
        role === "student"
          ? API.get("/meals/my-bookings").catch(() => ({ data: [] }))
  ...
  const headers = {
    admin: {
      ...
      desc: "Global system performance and resource efficiency",
    },
    staff: {
      title: "Kitchen Control",
      desc: "Live canteen waste analytics and demand forecasting",
    },
    ngo: { title: "Distribution Hub", desc: "Logistics tracking and community impact dashboard" },
    student: {
      title: "Community Support Hub",
      desc: "Access daily meal resources and track your ecological impact",
    },
  ...
  const allStatCards = {
    ...
    student: [
      ...
      {
        label: "Karma Points",
        value: (stats.volunteerCount || 0) * 100,
        icon: "🙏",
        color: "text-rose-500",
        bg: "bg-rose-50 dark:bg-rose-900/20",
        border: "border-rose-100 dark:border-rose-900/30",
  ...
  const renderGoalBanner = () => {
    ...
    if (["staff", "admin"].includes(role)) {
      ...
      goalParams = {
        title: "Community Impact: Reach 5 Donations",
        progress: 20,
        label: "1 / 5 contributions",
      };
    } else if (role === "ngo") {
      ...
      goalParams = {
        title: `Eco-Warrior Level ${level}`,
        progress: progressToNext,
        label: `${500 - ((user?.ecoScore || 0) % 500)} pts to Level ${level + 1}`,
      };
    }
    ...
    return (
      <div className="bg-gradient-to-r from-emerald-500 to-teal-600 rounded-3xl p-6 sm:p-8 mb-10 text-white shadow-xl relative overflow-hidden group">
        <div className="absolute top-0 right-0 p-8 opacity-10 text-8xl transform group-hover:scale-110 transition-transform">
          🏆
        </div>
        <h3 className="text-xl font-black mb-2 uppercase tracking-tight">{goalParams.title}</h3>
        <p className="text-emerald-100 text-sm mb-5 font-bold opacity-80 uppercase tracking-widest">
          Global impact tracking in progress
        </p>
        <div className="w-full bg-black/20 rounded-full h-4 mb-2 overflow-hidden shadow-inner">
          <div
            className="bg-white rounded-full h-4 transition-all duration-1000 ease-out shadow-sm"
            style={{ width: `${Math.min(goalParams.progress, 100)}%` }}
          ></div>
        </div>
        <p className="text-[10px] font-black text-right uppercase tracking-widest">
          {goalParams.label}
        </p>
      </div>
    );
    ...
    <div className="flex min-h-[calc(100vh-64px)] bg-slate-50 dark:bg-slate-900 transition-colors relative overflow-hidden mesh-gradient">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div
          ...
          className="absolute top-[-10%] left-[-5%] w-[40rem] h-[40rem] bg-emerald-400/15 dark:bg-emerald-500/10 rounded-full mix-blend-soft-light dark:mix-blend-multiply filter blur-[100px]"
        />
        <motion.div
          animate={{ x: [0, -50, 0], y: [0, 80, 0] }}
          transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
          className="absolute bottom-[-15%] right-[-5%] w-[45rem] h-[45rem] bg-teal-400/15 dark:bg-teal-500/10 rounded-full mix-blend-soft-light dark:mix-blend-multiply filter blur-[120px]"
        />
      </div>
        ...
        >
          <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            <div className="hidden md:block text-right">
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest print:hidden">
                System Uplink
              </p>
              <div className="flex items-center justify-end gap-2 mt-1 print:hidden">
                <button
                  onClick={() => window.print()}
                  className="px-3 py-1 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 text-[10px] font-black uppercase tracking-widest rounded-lg flex items-center gap-1 transition-colors hover:bg-emerald-100 dark:hover:bg-emerald-900/50 shadow-sm"
                >
                  📊 Export Report
                </button>
                <div className="relative group/status flex items-center gap-2 mt-1 print:hidden">
                  <div className="absolute -inset-1 bg-emerald-500/20 rounded-full blur group-hover/status:bg-emerald-500/40 transition-all animate-pulse"></div>
                  <div className="relative w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]"></div>
                  <p className="relative text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-[0.2em]">
            ...
            {role === "student" && stats.myMeals?.length > 0 && (
              <div className="md:col-span-2 lg:col-span-4">
                <h2 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.3em] mb-6 flex items-center gap-3">
                  <span className="w-8 h-px bg-slate-200 dark:bg-slate-700"></span> MY ACTIVE
                  BOOKINGS
                </h2>
                <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
                  {stats.myMeals.map((m) => (
                    <div
                      key={m._id}
                      className="min-w-[280px] bg-white dark:bg-slate-800/60 backdrop-blur-sm rounded-[2rem] p-6 border border-slate-200 dark:border-white/5 shadow-xl flex items-center justify-between group"
                    >
                      <div>
                        <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">
                          {m.mealType}
                        </p>
                        <p className="text-sm font-black text-slate-900 dark:text-white mt-1 uppercase tracking-tight">
                      ...
                      <button
                        onClick={async () => {
                          try {
                            ...
                            toast("Gifting failed", "error");
                          }
                        }}
                        className="px-5 py-2.5 bg-indigo-500 hover:bg-emerald-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-indigo-500/20 group-hover:scale-105"
                      >
                        Gift Meal
                      </button>
                    </div>
            ...
            {role === "student" && stats.rawDonations?.some((d) => d.status === "Assigned") && (
              <div className="md:col-span-2 lg:col-span-4 mb-4">
                <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-[3rem] p-8 sm:p-10 text-white relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-10 opacity-10 text-9xl transform group-hover:rotate-12 transition-transform">
                    🚚
                  </div>
                  <div className="relative z-10 max-w-xl">
                    <span className="bg-white/20 text-white text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-[0.2em] mb-6 inline-block">
                      Flash Opportunity
                    </span>
                    <h2 className="text-3xl font-black mb-3 tracking-tighter uppercase">
                      Volunteers Needed Nearby!
                    </h2>
                    <p className="text-indigo-100 text-sm font-bold opacity-80 mb-8 leading-relaxed uppercase tracking-tight">
                      An NGO partner is arriving to collect a surplus donation. Help with the
                      hand-off to earn 100 Karma Points!
                    </p>
                    <button
                      onClick={() => toast("You've been added to the volunteer list!", "success")}
                      className="px-10 py-4 bg-white text-indigo-600 text-xs font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-emerald-400 hover:text-white transition-all shadow-2xl active:scale-95"
                    >
              ...
              >
                {statCards.map((card, idx) => (
                  <motion.div
                    ...
                    className="spotlight-card bg-white dark:bg-slate-800/60 backdrop-blur-sm rounded-[2rem] border border-slate-200 dark:border-white/5 p-6 flex flex-col sm:flex-row items-center sm:items-start gap-4 hover:border-emerald-500/30 transition-all shadow-xl group cursor-default"
                  >
                    <div
                      className={`${card.bg} border ${card.border} rounded-2xl p-4 text-3xl group-hover:rotate-12 transition-transform shadow-inner`}
                    >
                      {card.icon}
                    </div>
                    <div>
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 font-black tracking-widest uppercase mb-1">
                        {card.label}
                      </p>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                <div className="xl:col-span-2 space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {["admin", "staff"].includes(role) && (
                      <>
                        <div className="bg-white dark:bg-slate-800/60 backdrop-blur-sm rounded-[2rem] p-6 border border-slate-200 dark:border-white/5 shadow-xl overflow-hidden group hover:border-emerald-500/30 transition-all">
                          <WasteChart />
                        </div>
                        <div className="bg-white dark:bg-slate-800/60 backdrop-blur-sm rounded-[2rem] p-6 border border-slate-200 dark:border-white/5 shadow-xl overflow-hidden group hover:border-emerald-500/30 transition-all">
                          <ExpiryChart />
                    ...
                    {role === "student" && (
                      <div className="md:col-span-2">
                        <AchievementBadges userAchievements={stats.achievements} />
                      </div>
                    )}
                  </div>
                </div>
                <div className="xl:col-span-1">
                  {role === "student" ? (
                    ...
                    <CanteenInsights
                      stats={{ ...stats, donationsList: stats.rawDonations || [] }}
                      userRole={role}
                    />
                  )}
                </div>
          ...
          <div className="mt-14">
            <h2 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.3em] mb-8 flex items-center gap-3">
              <span className="w-8 h-px bg-slate-200 dark:bg-slate-700"></span> SYSTEM INTEL
            </h2>
              ...
              {role === "student" ? (
                <>
                  <div className="bg-amber-500/5 backdrop-blur-sm border border-amber-500/10 p-8 rounded-[2rem] hover:border-amber-500/40 transition-all group">
                    ...
                    <p className="font-black text-amber-900 dark:text-white uppercase text-xs tracking-widest">
                      Peak Detection
                    </p>
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 font-bold leading-relaxed tracking-tight">
                      Canteen rush expected at 12:45 PM. Plan your visit accordingly.
                    </p>
                  </div>
                  <div className="bg-emerald-500/5 backdrop-blur-sm border border-emerald-500/10 p-8 rounded-[2rem] hover:border-emerald-500/40 transition-all group">
                    <p className="text-3xl mb-6 shadow-inner">🍱</p>
                    <p className="font-black text-emerald-900 dark:text-white uppercase text-xs tracking-widest">
                      Surplus Save
                    </p>
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 font-bold leading-relaxed tracking-tight">
                      Lunch has low bookings today! High chance of surplus redistribution.
                    </p>
                  </div>
                  <div className="bg-indigo-500/5 backdrop-blur-sm border border-indigo-500/10 p-8 rounded-[2rem] hover:border-indigo-500/40 transition-all group">
                    <p className="text-3xl mb-6 shadow-inner">🌱</p>
                    <p className="font-black text-indigo-900 dark:text-white uppercase text-xs tracking-widest">
                      Eco Impact
                    </p>
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 font-bold leading-relaxed tracking-tight">
                      Your meal choices saved 4.2kg of CO2 this week. Keep it up!
                    </p>
                  </div>
                </>
              ) : role === "ngo" ? (
                <>
                  <div className="bg-indigo-500/5 backdrop-blur-sm border border-indigo-500/20 p-8 rounded-[2rem] hover:border-indigo-500/40 transition-all group">
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 font-bold leading-relaxed tracking-tight">
                      5 pending donations detected within a 2km radius of your route.
                    </p>
                  </div>
                  <div className="bg-emerald-500/5 backdrop-blur-sm border border-emerald-500/20 p-8 rounded-[2rem] hover:border-emerald-500/40 transition-all group">
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 font-bold leading-relaxed tracking-tight">
                      Avg. pickup time reduced to 18 mins. Your fleet is optimized.
                    </p>
                  </div>
                  <div className="bg-amber-500/5 backdrop-blur-sm border border-amber-500/20 p-8 rounded-[2rem] hover:border-amber-500/40 transition-all group">
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 font-bold leading-relaxed tracking-tight">
                      340 meals successfully redistributed in your sector this month.
                    </p>
                  </div>
                </>
              ) : role === "donor" ? (
                <>
                  <div className="bg-indigo-500/5 backdrop-blur-sm border border-indigo-500/20 p-8 rounded-[2rem] hover:border-indigo-500/40 transition-all group">
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 font-bold leading-relaxed tracking-tight">
                      Your contributions reached 12 local NGOs this week. Impact!
                    </p>
                  </div>
                  <div className="bg-amber-500/5 backdrop-blur-sm border border-amber-500/20 p-8 rounded-[2rem] hover:border-amber-500/40 transition-all group">
                    <p className="text-3xl mb-6 shadow-inner">⚡</p>
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 font-bold leading-relaxed tracking-tight">
                      3 NGOs currently seeking surplus in your immediate vicinity.
                    </p>
                  </div>
                  <div className="bg-emerald-500/5 backdrop-blur-sm border border-emerald-500/20 p-8 rounded-[2rem] hover:border-emerald-500/40 transition-all group">
                    <p className="text-3xl mb-6 shadow-inner">📦</p>
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 font-bold leading-relaxed tracking-tight">
                      All scheduled pickups are on time. Driver 'Rajesh' is 5 mins away.
                    </p>
                  </div>
                </>
              ) : (
                <>
                  <div className="bg-amber-500/5 backdrop-blur-sm border border-amber-500/20 p-8 rounded-[2rem] hover:border-amber-500/40 transition-all group">
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 font-bold leading-relaxed tracking-tight">
                      High traffic predicted for Tuesday Lunch. Adjust preparation.
                    </p>
                  </div>
                  <div className="bg-indigo-500/5 backdrop-blur-sm border border-indigo-500/20 p-8 rounded-[2rem] hover:border-indigo-500/40 transition-all group">
                    <p className="text-3xl mb-6 shadow-inner">🧪</p>
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 font-bold leading-relaxed tracking-tight">
                      Optimal levels. 0 critical expiries detected in next 48h.
                    </p>
                  </div>
                  <div className="bg-emerald-500/5 backdrop-blur-sm border border-emerald-500/20 p-8 rounded-[2rem] hover:border-emerald-500/40 transition-all group">
                    <p className="text-3xl mb-6 shadow-inner">📈</p>
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 font-bold leading-relaxed tracking-tight">
                      Canteen efficiency up by 12% following menu optimization.
                    </p>
                  </div>
                </>
              )}
