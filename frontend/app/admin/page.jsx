...
export default function AdminPanelPage() {
  ...
  if (loading) return (
     <div className="flex min-h-[calc(100vh-64px)] bg-slate-50 dark:bg-slate-900 items-center justify-center transition-colors">
      ...
      <div className="flex-1 overflow-y-auto h-[calc(100vh-64px)] px-6 lg:px-10 py-4 lg:py-6 relative z-10 transition-all duration-200">
        <div className="max-w-7xl mx-auto space-y-10">
          ...
          <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
             <div>
                <p className="text-[10px] font-black text-violet-600 dark:text-violet-500 uppercase tracking-[0.3em] mb-2">Executive Command</p>
                <h1 className="text-4xl lg:text-5xl font-black text-slate-900 dark:text-white tracking-tighter">Admin Panel</h1>
                <p className="text-slate-500 dark:text-slate-400 mt-2 font-bold text-sm leading-relaxed max-w-xl">Manage system users, roles, and send notices. Available to Campus Administrators only.</p>
             </div>
          </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              { label: "Active Access", val: users.filter(u => u.status === "active").length, color: "emerald" },
              { label: "Suspended", val: users.filter(u => u.status === "suspended").length, color: "rose" },
            ].map(s => (
              <div key={s.label} className={`bg-white dark:bg-slate-800/60 backdrop-blur-sm p-8 rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-xl group hover:border-${s.color}-500/30 transition-all hover:-translate-y-1`}>
                <p className={`text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.3em] mb-1 ${s.color === "rose" ? "text-rose-500/80" : ""}`}>{s.label}</p>
                <p className={`text-4xl font-black text-slate-900 dark:text-white tracking-tighter`}>{s.val}</p>
                <div className={`mt-6 h-1 w-16 bg-${s.color}-500 rounded-full group-hover:w-full transition-all`}></div>
              </div>
            ))}
          </div>
          ...
          {users.map((u, i) => (
            <motion.div key={u._id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
              ...
              <div className="flex items-center gap-5">
                <div className="w-12 h-12 rounded-[1.5rem] bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center text-white font-black text-lg shadow-lg shadow-violet-500/20 group-hover:scale-110 group-hover:rotate-6 transition-all">
                  {u.name?.[0] || 'U'}
                </div>
                <div>
                  <p className="font-bold text-slate-800 dark:text-white text-md tracking-tight">{u.name}</p>
                  <p className="text-xs text-slate-500 font-medium">{u.email}</p>
                </div>
              </div>

              <div className="flex flex-wrap items-center gap-4 justify-end">
                <span className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${roleColors[u.role] || "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300 border border-slate-200 dark:border-white/5"}`}>
                  {u.role}
                </span>
                <span className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${u.status === "active" ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20" : "bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20 animate-pulse"}`}>
                  {u.status}
                </span>
                ...
                <div className="flex items-center gap-2 border-l border-slate-200 dark:border-slate-700 pl-4">
                  ...
                  <button onClick={() => toggleStatus(u._id, u.status)} title={u.status === "active" ? "Suspend" : "Activate"}
                    ...
                    {u.status === "active" ? <UserX size={16} strokeWidth={2.5} /> : <UserCheck size={16} strokeWidth={2.5} />}
                  </button>
                  <button onClick={() => handleDelete(u._id, u.name)} title="Permanently Delete User"
