      ...
      <div className="flex-1 overflow-y-auto h-[calc(100vh-64px)] p-6 lg:p-10 relative z-10">
        ...
        >
          <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <p className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-[0.3em] mb-2 leading-none">
                Collaboration Hub
              </p>
              ...
              <p className="text-slate-500 dark:text-slate-400 mt-2 font-bold text-sm leading-relaxed max-w-xl">
                Coordinate with donor organizations and logistics partners across the network.
              </p>
            </div>
              ...
              {filteredPartners.map((partner, i) => (
                ...
                >
                  <div className="flex items-start justify-between mb-8">
                    ...
                    <span
                      className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${roleColors[partner.role]}`}
                    >
                      {partner.role}
                    </span>
                  </div>

                  <h3 className="text-xl font-black text-slate-800 dark:text-white mb-2 truncate">
                    {partner.name}
                  </h3>
          ...
          {!loading && filteredPartners.length === 0 && (
            <div className="text-center py-20 bg-white/40 dark:bg-white/5 backdrop-blur-md rounded-[3rem] border border-white dark:border-white/5">
              ...
              <h3 className="text-xl font-black text-slate-800 dark:text-white mb-2 uppercase tracking-wide">
                No Partners Found
              </h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 font-bold max-w-xs mx-auto uppercase tracking-tight opacity-70">
                Expand our network by inviting more local donors and NGOs to join the mission.
              </p>
            </div>
          )}
        </motion.div>
