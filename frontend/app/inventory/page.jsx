    ...
    <div className="flex min-h-[calc(100vh-64px)] bg-slate-50 dark:bg-slate-900 transition-colors relative overflow-hidden mesh-gradient">
      ...
      <div className="flex-1 overflow-y-auto h-[calc(100vh-64px)] px-6 lg:px-10 py-4 lg:py-6 relative z-10 transition-all duration-200">
        ...
        >
          ...
          >
             <div>
                <p className="text-[10px] font-black text-amber-600 dark:text-amber-500 uppercase tracking-[0.3em] mb-2">Operations Center</p>
                <h1 className="text-4xl lg:text-5xl font-black text-slate-900 dark:text-white tracking-tighter">Pantry & Inventory</h1>
                <p className="text-slate-500 dark:text-slate-400 mt-2 font-bold text-sm leading-relaxed max-w-xl">Manage stock levels, track ingredient expiry dates, and optimize procurement.</p>
             </div>
             <motion.div variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }} className="hidden md:block text-right">
                <p className="text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">Inventory Status</p>
                <div className="flex items-center justify-end gap-2 mt-1">
                   {expiryAlerts.length > 0 ? (
                     <>
                       <div className="w-2 h-2 rounded-full bg-rose-500 animate-ping"></div>
                       <p className="text-xs font-bold text-rose-600 uppercase tracking-widest">Attention Required</p>
                     </>
                   ) : (
                     <>
                       <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                       <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">Optimal</p>
                     </>
                   )}
                </div>
             </motion.div>
          ...
          >
            {['All', 'Dairy', 'Vegetables', 'Grains'].map((cat) => (
              <div 
                key={cat}
              ...
              >
                 ...
                 <p className={`text-xl font-black uppercase tracking-tighter ${filter === cat ? 'text-emerald-500' : 'text-slate-700 dark:text-slate-300'}`}>{cat}</p>
              </div>
            ))}
          </motion.div>
